...
bool COpencvprocess::CopyMatDataToMem(const Mat& src, uchar **data)
{
	//四字节对齐
...
cv::Rect COpencvprocess::GetROI(cv::Rect ScreenSize, cv::Rect DrawROI, cv::Rect ImgSize)
{
	cv::Rect dst;
	double ratio_x = (double)ImgSize.width / (double)ScreenSize.width;
	double ratio_y = (double)ImgSize.height / (double)ScreenSize.height;
	dst.width = static_cast<int>(ScreenSize.width * ratio_x);
	dst.height = static_cast<int>(ScreenSize.height*ratio_y);
	dst.x = static_cast<int>(DrawROI.x * ratio_x);
	dst.y = static_cast<int>(DrawROI.y * ratio_y);
	return dst;
}

void COpencvprocess::SetWindow(size_t width, size_t height, void* winhandle)
{
	winwidth = width;
	winheight = height;
	return;
	...
	w.w = width;
	w.h = height;
	w.winhandle = winhandle;
	Window.push_back(w);
}


//HDR
cv::Mat  COpencvprocess::hdr(const std::vector<cv::Mat> & images, cv::Mat& dst, const std::vector<float>& times, cv::Mat respose)
{
	Ptr<MergeDebevec> merge_debevec = createMergeDebevec();
	merge_debevec->process(images,dst,times,respose);
	return dst;
}

cv::Mat  COpencvprocess::hdr(const std::vector<cv::Mat>images, const std::vector<float>& times, cv::Mat response)
{
	cv::Mat dst;
	Ptr<MergeDebevec> merge_debevec = createMergeDebevec();
	merge_debevec->process(images, dst, times, response);
	return dst;
}

//ldr
cv::Mat  COpencvprocess::ldr(const std::vector<cv::Mat> & images, cv::Mat& dst, const std::vector<float>& times, cv::Mat response)
{
	Ptr<TonemapDurand> tonemap = createTonemapDurand(2.2f);
	tonemap->process(hdr(images,times,response), dst);
	return dst;
}

cv::Mat  COpencvprocess::ldr(const std::vector<cv::Mat> & images, const std::vector<float>& times, cv::Mat response)
{
	cv::Mat dst;
	Ptr<TonemapDurand> tonemap = createTonemapDurand(2.2f);
	tonemap->process(hdr(images, times, response), dst);
...
cv::Mat  COpencvprocess::fusion(const std::vector<cv::Mat> images, cv::Mat & dst)
{
	Ptr<MergeMertens> merge_mertens = createMergeMertens();
	merge_mertens->process(images, dst);
	return dst;
}

cv::Mat  COpencvprocess::fusion(const std::vector<cv::Mat> images)
{
	cv::Mat dst;
	Ptr<MergeMertens> merge_mertens = createMergeMertens();
	merge_mertens->process(images, dst);
...
cv::Mat COpencvprocess::GetHSVData(const cv::Mat & src, cv::Rect rect)
{
	...
	cvtColor(rect_img, rect_hsv, COLOR_BGR2HSV_FULL);
	exsplit(rect_hsv,mv);
	cv::Mat data(1, 3,CV_32FC1);
	data.at<float>(0, 0) = average_data(mv[0]);
	data.at<float>(0, 1) = average_data(mv[1]);
	data.at<float>(0, 2) = average_data(mv[2]);
	return data;
}

int COpencvprocess::ann_train(const std::vector<std::string>& front_images, const std::vector<std::string>& back_images)
{
	const size_t samples_quantity = front_images.size() + back_images.size();
	size_t counter = 0;
	//HSV has 3 datas
	Mat_<float> data(samples_quantity, 3);
	Mat_<float> responses(samples_quantity, 2);
	size_t samples_index = 0;
	//fill front_images data;
	for (; samples_index < front_images.size(); samples_index++)
	{
		cv::Mat src = cv::imread(front_images[samples_index].c_str(), -1);
		if (src.empty()) continue;
		cv::Mat hsvdata = GetHSVData(src, cv::Rect(0, 0, src.cols, src.rows));
		data(samples_index, 0) = hsvdata.at<float>(0, 0);
		data(samples_index, 1) = hsvdata.at<float>(0, 1);
		data(samples_index, 2) = hsvdata.at<float>(0, 2);
		responses(samples_index, 0) = 0;
		responses(samples_index, 1) = 1;
		counter++;
	}
	//fill back_images data
	for (int i = 0; samples_index < samples_quantity; samples_index++,i++)
	{
		...
		data(samples_index, 1) = hsvdata.at<float>(0, 1);
		data(samples_index, 2) = hsvdata.at<float>(0, 2);
		responses(samples_index, 0) = 1;
		responses(samples_index, 1) = 0;
		counter++;
	} 
	if (samples_quantity != counter || 0 == counter)
	{
#ifdef _DEBUG
		cout << "no pic found" << endl;
		return 2;
#endif
	}
	color_network = color_train(data,responses);
	if (color_network->isTrained())
	{
		color_network->save("color.xml");
		return 0;
	}
	return 1;
}

bool  COpencvprocess::ann_load(const String filename)
{
	color_network = Algorithm::load<ANN_MLP>(filename);
	return true;
}

//颜色分类器训练
Ptr<ANN_MLP> COpencvprocess::color_train(const Mat_<float> &data, const Mat_<float>& responses)
{
...
//颜色分类器
float COpencvprocess::color_predict(Mat_<float>& data)
{
	try {
		cv::Mat result;
		color_network->predict(data.row(0), result);
...
float  COpencvprocess::color_predict(const cv::Mat & src)
{
	cv::Mat_<float> data(1, 3);
	cv::Mat hsvdata = GetHSVData(src, cv::Rect(0, 0, src.cols, src.rows));
	data(0, 0) = hsvdata.at<float>(0, 0);
	data(0, 1) = hsvdata.at<float>(0, 1);
	data(0, 2) = hsvdata.at<float>(0, 2);
	cv::Mat result;
	color_network->predict(data.row(0), result);
	cout << "result is:" << result << endl;
	return -1.0;
