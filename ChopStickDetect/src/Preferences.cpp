...
void CPreferences::LoadParaFile(UINT item)
{
	...
	if (ReadFile())
	{
		...
		if (PARA_PRJ == (mask&PARA_PRJ))
		{
			...
			m_front_path = GetValue(L"projectSetting", L"FRONTPATH");
			m_back_path = GetValue(L"projectSetting", L"BACKPATH");
...
void CPreferences::SaveParaFile(UINT item)
{
	...
	{
		//m_strCurrentProjectName = GetValue(L"projectSetting",L"CurrentProject");
		SetValue(L"projectSetting", L"FRONTPATH", m_front_path);
		SetValue(L"projectSetting", L"BACKPATH", m_back_path);
...
void CPreferences::initData()
{
	...
	m_front_path = L"COLOROK";
	m_back_path = L"COLORNG";
	m_shutter = 2500.0;
	m_idCamera = 0;
